...
//default values
#define DMS_ASYNC_DEFAULT_PERSISTENCE	MQTTCLIENT_PERSISTENCE_NONE
#define DMS_ASYNC_DEFAULT_KEEPALIVE		40
#define DMS_ASYNC_DEFAULT_RETRY			60
#define DMS_ASYNC_DEFAULT_CLEANSESSION	0
#define DMS_ASYNC_DEFAULT_QOS			1
#define DMS_ASYNC_DEFAULT_RETAINED		0
#define DMS_ASYNC_DEFAULT_DISCON_TIMEOUT 0

//函数返回
#define DMSASYNC_SUCCESS 0
#define DMSASYNC_FAILURE -1

//类型重定义
typedef MQTTAsync DMSAsync;
typedef MQTTAsync_message DMSAsync_message;
typedef MQTTAsync_token DMSAsync_token;
typedef MQTTAsync_successData DMSAsync_successData;
typedef MQTTAsync_failureData DMSAsync_failureData;
typedef MQTTAsync_responseOptions DMSAsync_responseOptions;
typedef MQTTAsync_disconnectOptions DMSAsync_disconnectOptions;
typedef void DMSAsync_connectionLost(void* context, char* cause);
/** DMSAsync_messageArrived
* 返回 false 表示接收消息失败，会再次调用DMSAsync_messageArrived；如果成功处理了消息，请返回true,即非0值
*/
typedef int DMSAsync_messageArrived(void* context, char* topicName, int topicLen, DMSAsync_message* message);
typedef void DMSAsync_deliveryComplete(void* context, DMSAsync_token token);
typedef void DMSAsync_onSuccess(void* context, DMSAsync_successData* response);
typedef void DMSAsync_onFailure(void* context, DMSAsync_failureData* response);

/**
* 创建 DMSAsync，成功返回 DMSAsync，失败返回NULL
*/
DLLExport DMSAsync DMSAsync_create(const char* serverURI, const char* clientId);

/** 设置回调函数
* 成功返回 DMSASYNC_SUCCESS，失败返回 DMSASYNC_FAILURE
*/
DLLExport int DMSAsync_setCallbacks(DMSAsync handle, void* context, DMSAsync_connectionLost* cl, DMSAsync_messageArrived* ma, DMSAsync_deliveryComplete* dc);

/** 建立连接 
* 成功返回 DMSASYNC_SUCCESS，否则onFailure回调函数中返回错误代码
*/
DLLExport int DMSAsync_connect(DMSAsync handle, const char* pub_key, const char* sub_key,
						DMSAsync_onSuccess* onSuccess, DMSAsync_onFailure* onFailure, void* context);

/** 关注话题
* 成功返回 DMSASYNC_SUCCESS，否则返回错误代码
*/
DLLExport int DMSAsync_subscribe(DMSAsync handle, const char* topic, DMSAsync_responseOptions* response);
/** 取消关注
* 成功返回 DMSASYNC_SUCCESS，否则返回错误代码
*/
DLLExport int DMSAsync_unsubscribe(DMSAsync handle, const char* topic, DMSAsync_responseOptions* response);

#define RespOptsInit(onSuccess,onFailure,context) { {'M', 'Q', 'T', 'R'}, 0, (onSuccess), (onFailure), (context), 0 }
#define MessageInit(payloadlen,payload)	{ {'M', 'Q', 'T', 'M'}, 0, (payloadlen), (payload), DMS_ASYNC_DEFAULT_QOS, DMS_ASYNC_DEFAULT_RETAINED, 0, 0 }
/** 推送消息
* 成功返回 DMSASYNC_SUCCESS，否则返回错误代码
*/
DLLExport int DMSAsync_send(DMSAsync handle, const char* topic, int payloadlen, void* payload, DMSAsync_responseOptions* response);

/** 断开连接
* 成功返回 DMSASYNC_SUCCESS，否则返回错误代码
*/
#define DisconOptsInit(onSuccess,onFailure,context) { {'M', 'Q', 'T', 'D'}, 0, DMS_ASYNC_DEFAULT_DISCON_TIMEOUT, (onSuccess), (onFailure), (context) }
DLLExport int DMSAsync_disconnect(DMSAsync handle, const DMSAsync_disconnectOptions* options);

//销毁对象
DLLExport void DMSAsync_destroy(DMSAsync* handle);

//释放消息
DLLExport void DMSAsync_freeMessage(DMSAsync_message** msg);
//释放内存
DLLExport void DMSAsync_free(void* ptr);
//操作是否完成
DLLExport int DMSAsync_isComplete(DMSAsync handle, DMSAsync_token dt);
//等待操作完成
DLLExport int DMSAsync_waitForCompletion(DMSAsync handle, DMSAsync_token dt, unsigned long timeout);
